...
function obterColID(nome_col) {
	...
	var id_col = 0;
	var sql = "SELECT ID_cargo FROM TblCurriculo WHERE NOME_sobrenome = " + 
									     	"'" + nome_col + "'";
	  console.log(sql);
	  db.all(sql, [],  (err, rows ) => {
		  if (err) {
		  ...
		  id_col = rows[0]['ID_cargo'];
		  console.log("Objeto 0 - ID_cargo:" + rows[0]['ID_cargo']);
	  });
...
app.get('/curriculo', (req, res) => {
	...
	console.log("EndPoint /curriculo");
	var db = new sqlite3.Database(DBPATH); // Abre o banco
    var sql = 'SELECT * FROM Tblcurriculo';
	db.all(sql, [],  (err, rows ) => {
		if (err) {
		    throw err;
		}
...
app.post('/colinsert', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); // Isso é importante para evitar o erro de CORS	
	sql = "INSERT INTO Tblcurriculo (ID_cargo, NOME_sobrenome) " + 
	                      "VALUES (" + req.body.id  + 
						       ", '" + req.body.nome + "')";
	console.log(sql);
	var db = new sqlite3.Database(DBPATH); // Abre o banco
...
app.post('/protinsert', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); // Isso é importante para evitar o erro de CORS
	sql = "INSERT INTO Tblcurriculo (ID_cargo,ID_cargo, NOME_sobrenome) " + 
...
app.post('/colupdate', urlencodedParser, (req, res) => {
	...
	sql = "UPDATE Tblcurriculo SET nome = '" + req.body.nome + 
	    "' WHERE ID_cargo = " + req.body.id;
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.post('/coldelete', urlencodedParser, (req, res) => {
	...
	sql = "DELETE FROM Tblcurriculo WHERE ID_cargo = " + req.body.id;
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.post('/coldelete_nome', urlencodedParser, (req, res) => {
	res.statusCode = 200;
	res.setHeader('Access-Control-Allow-Origin', '*'); // Isso é importante para evitar o erro de CORS
	sql = "DELETE FROM Tblcurriculo WHERE NOME_sobrenome = '" + req.body.nome_col + "'";
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.post('/protdelete', urlencodedParser, (req, res) => {
	...
	sql = "DELETE FROM Tblcurriculo WHERE ID_cargo= " + req.body.id_prot;
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.post('/protdeletecol', urlencodedParser, (req, res) => {
	...
	res.setHeader('Access-Control-Allow-Origin', '*'); // Isso é importante para evitar o erro de CORS
	var idcol = obterColID(req.body.nome_col);	
	sql = "DELETE FROM Tblcargo WHERE ID_cargo = " + idcol;
	var db = new sqlite3.Database(DBPATH); // Abre o banco
	db.run(sql, [],  err => {
		if (err) {
		    throw err;
...
app.listen(port, hostname, () => {
  ...
  console.log(`Page server running at http://${hostname}:${port}/curriculo`);
  console.log(`Page server running at http://${hostname}:${port}/curriculo`);
